 ...
 * @typedef {'monthly' | 'weekly' | 'biweekly' | 'every_4_weeks' | 'quarterly' | 'yearly' | 'custom_days'} PaymentFrequencyCode
 */
...
function getRelativeDateLabel(paymentIndex, frequency, offsetDays) {
  ...
  // Calculate total offset for this payment
  let totalOffset;

  if (frequency === 'monthly') {
    const months = isFirstPayment ? Math.floor(offsetDays / 30) : (paymentIndex - 1);
    if (months === 0 && isFirstPayment && offsetDays === 0) {
    ...
    const weeks = isFirstPayment ? Math.floor(offsetDays / 7) : (paymentIndex - 1);
    if (weeks === 0 && isFirstPayment && offsetDays === 0) {
      return 'On loan start';
    }
    if (weeks === 1) {
    ...
    const periods = isFirstPayment ? Math.floor(offsetDays / 14) : (paymentIndex - 1);
    ...
    const periods = isFirstPayment ? Math.floor(offsetDays / 28) : (paymentIndex - 1);
    ...
    const months = isFirstPayment ? Math.floor(offsetDays / 90) * 3 : (paymentIndex - 1) * 3;
    if (months === 0 && isFirstPayment && offsetDays === 0) {
      return 'On loan start';
    }
    if (months === 3) {
    ...
    const years = isFirstPayment ? Math.floor(offsetDays / 365) : (paymentIndex - 1);
    if (years === 0 && isFirstPayment && offsetDays === 0) {
      return 'On loan start';
    }
    if (years === 1) {
    ...
    const days = isFirstPayment ? offsetDays : (paymentIndex - 1) * offsetDays;
    if (days === 0 && isFirstPayment) {
      return 'On loan start';
    }
    if (days === 1) {
