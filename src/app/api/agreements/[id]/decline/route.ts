/**
 * POST /api/agreements/[id]/decline - Borrower declines the agreement
 */
...
export async function POST(
  ...
  try {
    ...
    // Only the borrower can decline
    if (agreement.borrower_user_id !== authUser.id) {
      return NextResponse.json(
        { error: 'Only the borrower can decline this agreement' },
        { status: 403 }
      );
    }

    // Can only decline pending agreements
    ...
    console.error('[API] Decline agreement error:', error);
    return NextResponse.json(
